...
int RayTraceRender::Render(SmartPointer<CameraBase> pCamera, SmartPointer<IWorld> pWorld)
{
	
	
	SmartPointer<RayTraceCamera> pRTCamera = pCamera.SmartPointerCast<RayTraceCamera>();
...
//primary ray
	...
	SmartPointer<IRenderable> pInterGeo = nullptr;
	float fInterDist = 1000000000;
	Vector3 vecInterPos = Vector3::ZERO;
	Vector3 vecNormal = Vector3::ONE;
	for each (SmartPointer<IRenderable> var in m_vecRenderables)
	{
	...
	if (pInterGeo != nullptr)
	{
		...
		SmartPointer<SimpleRTMaterial> pRTMat = pInterGeo->getRenderMaterial().SmartPointerCast<SimpleRTMaterial>();
		if (pRTMat == nullptr)
		{
		...
		else
		{
			...
			//shadow ray
			for each (SmartPointer<LightBase> var in m_vecLights)
			{
				vecNormal.normalize();
...
bool RayTraceRender::ShadowRay(const Ray3D& r, SmartPointer<LightBase> pLight)
{
